...
// Application-layer encryption for sensitive identity fields (passport numbers,
// national IDs, visa sticker numbers). Uses AES-256-GCM with a random 12-byte
// IV per value. Values are stored as `enc:v1:<iv>:<ciphertext>:<authTag>` so we
// can detect legacy plaintext rows (anything without the prefix) and pass them
// through untouched until they're next written.
